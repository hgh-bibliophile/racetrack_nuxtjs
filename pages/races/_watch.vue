        ...
        <div :class="color('bg', t_data.lane.color, '500')" class="flex-shrink w-full overflow-hidden shadow sm:rounded-lg sm:p-4 sm:my-2 p-3">
          <div class="p-3 sm:p-6 bg-white text-center rounded-lg">
            ...
            <h2 class="mb-3 text-2xl sm:text-5xl font-normal text-gray-700 dark:text-gray-400">
              <div v-if="t_data.run && !active_heat">{{ t_data.run.mph ? t_data.run.mph.toFixed(3) : 0 }} mph ~ {{ t_data.run.fps ? t_data.run.mph.toFixed(3) : 0 }} fps</div>
...
export default {
  ...
  methods: {
    ...
    connect () {
      this.ws = new WebSocket('ws://localhost:8000/watch/' + this.$route.params.watch)
      console.log(this.ws)
      this.ws.onmessage = (e) => {
